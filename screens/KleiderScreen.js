...
const MODI = [
  ...
  { id: 'suche',  label: 'Ich suche',  emoji: '🔍', hint: 'Etwas benötigt' },
...
async function pickImages(current, onDone) {
  ...
  try {
    ...
    if (!result.canceled) onDone([...current, ...result.assets.map(a => a.uri).slice(0, remaining)]);
  } catch { Alert.alert('Fehler', 'Fotos konnten nicht geladen werden.'); }
}
    ...
    <View style={ph.row}>
      {bilder.map((uri, i) => (
        <View key={i} style={ph.thumb}>
          ...
          <TouchableOpacity style={ph.removeBtn} onPress={() => onChange(bilder.filter((_, j) => j !== i))}>
            <Text style={ph.removeText}>✕</Text>
          </TouchableOpacity>
        </View>
      ))}
      {bilder.length < 3 && (
        <TouchableOpacity style={ph.addBtn} onPress={() => pickImages(bilder, onChange)}>
    ...
    <Modal visible={visible} animationType="slide" presentationStyle="pageSheet">
      <View style={dt.container}>
          ...
          {hasPhotos ? (
            <View style={dt.carousel}>
              ...
              {item.bilder.length > 1 && (
                <View style={dt.dots}>
                  {item.bilder.map((_, i) => <View key={i} style={[dt.dot, i === photoIdx && dt.dotActive]} />)}
                </View>
              )}
            </View>
          ) : (
          ...
          <View style={dt.content}>
            <View style={dt.badgeRow}>
              <View style={[dt.badge, { backgroundColor: modusColor.bg }]}>
                <Text style={[dt.badgeTxt, { color: modusColor.text }]}>{modus.emoji} {modus.label}</Text>
              </View>
              {item.groesse && <View style={dt.badge}><Text style={dt.badgeTxt}>Gr. {item.groesse}</Text></View>}
              <View style={dt.badge}><Text style={dt.badgeTxt}>{kat.emoji} {kat.label}</Text></View>
              {item.vergeben && (
            ...
            <View style={dt.metaRow}>
              <View style={dt.avatarSmall}><Text style={dt.avatarSmallTxt}>{item.autor[0]}</Text></View>
              <Text style={dt.meta}>{item.autor}</Text>
              <Text style={dt.metaDot}>·</Text>
              <Text style={dt.meta}>{relativeTime(item.ts)}</Text>
            </View>
            {!!item.text && <View style={dt.descBox}><Text style={dt.desc}>{item.text}</Text></View>}
            {isOwn && item.interessenten.length > 0 && (
              <View style={dt.interessentenBox}>
                <Text style={dt.interessentenTitle}>{item.interessenten.length} Person{item.interessenten.length !== 1 ? 'en' : ''} interessiert</Text>
                <View style={dt.interessentenList}>
                  {item.interessenten.map(name => (
                    <View key={name} style={dt.interessentRow}>
                      <View style={dt.avatarSmall}><Text style={dt.avatarSmallTxt}>{name[0]}</Text></View>
        ...
        <View style={dt.footer}>
          {isOwn ? (
            <TouchableOpacity style={[dt.actionBtn, item.vergeben && dt.actionBtnSolved]}
              onPress={() => { onToggleVergeben(item.id); onClose(); }}>
              <Text style={[dt.actionTxt, item.vergeben && dt.actionTxtSolved]}>
                {item.vergeben ? 'Wieder verfügbar markieren' : '✅ Als vergeben markieren'}
              </Text>
            </TouchableOpacity>
          ) : (
            <TouchableOpacity
              style={[dt.actionBtn, ichWill && dt.actionBtnActive, item.vergeben && dt.actionBtnDone]}
              onPress={() => { if (!item.vergeben) onToggleInteresse(item.id); }}
              activeOpacity={item.vergeben ? 1 : 0.75}
            >
              {ichWill ? (
                <Text style={dt.actionTxtActive}>Interesse gemeldet · Zurück ziehen</Text>
              ) : item.vergeben ? (
...
function NewKleiderModal({ visible, onSubmit, onClose, editItem }) {
  ...
  function reset() { setStep(1); setModus(null); setKategorie(null); setTitel(''); setText(''); setGroesse(null); setBilder([]); prevEdit.current = null; }
        ...
        <ScrollView style={{ flex: 1 }} keyboardShouldPersistTaps="handled" contentContainerStyle={fm.scrollContent}>
          {step === 1 && (
            <View style={fm.stepContent}>
              <Text style={fm.stepTitle}>Was möchtest du?</Text>
              <Text style={fm.stepSub}>Tippe auf eine Option</Text>
              <View style={fm.modiGrid}>
                {MODI.map(m => (
                  <TouchableOpacity key={m.id} style={[fm.modusTile, modus === m.id && fm.modusTileActive]}
                    onPress={() => { setModus(m.id); setStep(2); }} activeOpacity={0.75}>
          ...
          {step === 2 && (
            <View style={fm.stepContent}>
              <TouchableOpacity style={fm.backRow} onPress={() => setStep(1)}>
                <Text style={fm.backText}>{selectedModus?.emoji} {selectedModus?.label}  <Text style={fm.backChange}>ändern</Text></Text>
              </TouchableOpacity>
              <Text style={fm.stepTitle}>Was für ein Kleidungsstück?</Text>
              <View style={fm.katGrid}>
                {KATEGORIEN.map(k => (
                  <TouchableOpacity key={k.id} style={[fm.katTile, kategorie === k.id && fm.katTileActive]}
                    onPress={() => { setKategorie(k.id); setStep(3); }} activeOpacity={0.75}>
          ...
          {step === 3 && (
            <View style={fm.stepContent}>
              <View style={fm.breadcrumb}>
                <TouchableOpacity onPress={() => setStep(1)}><Text style={fm.breadcrumbItem}>{selectedModus?.emoji} {selectedModus?.label}</Text></TouchableOpacity>
                <Text style={fm.breadcrumbSep}>›</Text>
                <TouchableOpacity onPress={() => setStep(2)}><Text style={fm.breadcrumbItem}>{selectedKat?.emoji} {selectedKat?.label}</Text></TouchableOpacity>
              </View>
              <Text style={fm.stepTitle}>Details</Text>
              <Text style={fm.label}>Kurze Beschreibung *</Text>
              <TextInput style={fm.input} placeholder="z.B. Winterjacke Gr. 110, kaum getragen" value={titel} onChangeText={setTitel} autoFocus returnKeyType="next" />
              <Text style={fm.label}>Grösse</Text>
              <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={fm.groesseRow}>
                {GROESSEN.map(g => (
                  <TouchableOpacity key={g} style={[fm.groesseChip, groesse === g && fm.groesseChipActive]} onPress={() => setGroesse(groesse === g ? null : g)}>
                    <Text style={[fm.groesseText, groesse === g && fm.groesseTextActive]}>{g}</Text>
                  </TouchableOpacity>
                ))}
              </ScrollView>
              <Text style={fm.label}>Fotos <Text style={fm.labelOpt}>(max. 3)</Text></Text>
              <PhotoStrip bilder={bilder} onChange={setBilder} />
              <Text style={fm.label}>Mehr Details <Text style={fm.labelOpt}>(optional)</Text></Text>
              <TextInput style={[fm.input, fm.textarea]} placeholder="Farbe, Marke, Zustand, Abholort..." value={text} onChangeText={setText} multiline />
            </View>
          )}
        </ScrollView>
  ...
  return (
    <TouchableOpacity style={[cd.wrap, item.vergeben && cd.wrapDim]} onPress={onOpen} activeOpacity={0.88}>
        ...
        <View style={cd.photoWrap}>
          ...
          {item.bilder.length > 1 && (
            <View style={cd.photoThumbRow}>
              {item.bilder.slice(1).map((uri, i) => <Image key={i} source={{ uri }} style={cd.photoThumb} resizeMode="cover" />)}
            </View>
          )}
        </View>
      )}
      <View style={cd.top}>
        ...
        {item.vergeben ? (
          <View style={[cd.status, { backgroundColor: '#E8F5E9' }]}><Text style={[cd.statusTxt, { color: '#2E7D32' }]}>Vergeben</Text></View>
        ) : (
          <View style={[cd.status, { backgroundColor: modusColor.bg }]}><Text style={[cd.statusTxt, { color: modusColor.text }]}>{modus.emoji} {modus.label}</Text></View>
        )}
      </View>
      ...
      {isOwn ? (
        <View style={cd.ownerRow}>
          ...
          <TouchableOpacity style={[cd.manageBtn, item.vergeben && cd.manageBtnDone]} onPress={() => onToggleVergeben(item.id)}>
            <Text style={[cd.manageTxt, item.vergeben && cd.manageTxtDone]}>{item.vergeben ? 'Wieder verfügbar' : 'Als vergeben markieren'}</Text>
          </TouchableOpacity>
        </View>
      ) : (
        <TouchableOpacity style={[cd.interBtn, ichWill && cd.interBtnActive, item.vergeben && cd.interBtnDone]}
          onPress={() => !item.vergeben && onToggleInteresse(item.id)} activeOpacity={item.vergeben ? 1 : 0.75}>
          {ichWill ? <Text style={cd.interTxtActive}>Interesse gemeldet · Zurück ziehen</Text>
            : item.vergeben ? <Text style={cd.interTxtDone}>Bereits vergeben</Text>
            : item.modus === 'suche' ? <Text style={cd.interTxt}>🙋 Ich habe das!</Text>
...
// ─── Main Screen ──────────────────────────────────────────────────────────────
export default function KleiderScreen({ navigation, route }) {
  const { currentUser, inserate, addInserat, updateInserat, toggleInteresse, toggleVergeben } = useTratsch();
  ...
  useEffect(() => {
    const id = route?.params?.openInseratId;
    if (!id) return;
    const item = inserate.find(i => i.id === id);
    if (item) setDetailItem(item);
    navigation.setParams({ openInseratId: undefined });
  }, [route?.params?.openInseratId]);
  ...
  // Keep detailItem in sync with context changes
  const liveDetailItem = useMemo(
    () => detailItem ? inserate.find(i => i.id === detailItem.id) ?? null : null,
    [detailItem, inserate]
  );

  function handleSubmit({ modus, kategorie, titel, text, groesse, bilder }) {
    if (editItem) {
      updateInserat(editItem.id, { modus, kategorie, titel, text, groesse, bilder });
      setEditItem(null);
    } else {
      addInserat({ modus, kategorie, titel, text, groesse, bilder });
    }
  }

  function handleEdit(item) {
    setDetailItem(null);
    setTimeout(() => { setEditItem(item); setShowForm(true); }, 300);
  }
  ...
  return (
    <View style={sc.container}>
      ...
      <FlatList
        ...
        renderItem={({ item }) => (
          <KleiderCard item={item} currentUser={currentUser}
      ...
      <NewKleiderModal visible={showForm} editItem={editItem} onSubmit={handleSubmit}
        onClose={() => { setShowForm(false); setEditItem(null); }} />

      <DetailModal item={liveDetailItem} visible={!!liveDetailItem} currentUser={currentUser}
